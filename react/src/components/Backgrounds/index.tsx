...
import { BackgroundImage, BackgroundItem, StyledBackgrounds, BackgroundSelected, CheckboxParent, BackgroundsContainer, ColourPicker  } from "./style"
...
const SolidSettings = ({ defaultColour, colourChange }: { defaultColour: string; colourChange: any }) => {
    ...
    return (
        <>
            <CheckboxParent>
                ...
                >
                    <ColourPicker ref={colourRef} colour={colour} onClick={() => onClickColourPicker()} />
                    <div style={{ display: colourOpen ? "" : "none", transform: `translate(${bubblePos[0]}px, ${bubblePos[1]}px)`, position: "fixed", top: 0, right: 0, zIndex: 199 }}>
...
export const Backgrounds = () => {
    ...
    return (
        <StyledBackgrounds>
            ...
            {settings.provider == "solid" && <SolidSettings colourChange={onChangeColour} defaultColour={(!settings.colour || settings.colour.length == 0) ? "#2554e1" : settings.colour} />}
        </StyledBackgrounds>
    )
}